...
from pynhost.grammars.terminal import termutils
from pynhost.grammars import baseutils, extension

class HaskellCommandsGrammar(vimextension.VimExtensionGrammar):
    def __init__(self):
        super().__init__()
        self.mapping = {
        'function [<any> <1->]': self.new_function,
        '<hom_execute> <hom_program>': self.execute_program,
    ...
    def execute_program(self, words):
        api.send_string('zz')
        termutils.open_shell()
        current_dir = termutils.get_command_results('pwd')[0]
        filenames = termutils.get_command_results('ls')
        for filename in filenames:
            if len(filename) > 3 and filename[-3:] == '.hs':
                with open(os.path.join(current_dir, filename)) as f:
                    for line in f:
                        stripped = line.strip()
                        if re.match(r'main\s*=', stripped):
                            api.send_string('ghc {}'.format(filename))
                            return

                
        print(current_dir, filenames)
